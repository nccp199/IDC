...
def compute_bess_policy_diagnostics(
    *,
    ...
    full_ratio = np.prod(per_dim_ratio, axis=-1)
    effective_ratio = per_dim_ratio[:, 0]
    ...
    virtual_entropy = np.sum(entropy[:, 1:], axis=-1)
    ...
    return {
        ...
        "effective_entropy": float(np.mean(entropy[:, 0])),
        ...
        "effective_log_prob": float(np.mean(new[:, 0])),
        "virtual_log_prob_sum": float(np.mean(np.sum(new[:, 1:], axis=-1))),
